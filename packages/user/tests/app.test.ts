...
describe("Test authorize", () => {
  ...
  it("login POST /login", async () => {
    ...
    const res = await server.inject().post("/login").body(loginObj);
  ...
  it("returns token /login", async () => {
    const spy = vi
      .spyOn(userService, "getUserByCred")
...
describe("User controller", () => {
  ...
  it("returns unauthorized with empty token POST /users", async () => {
    const res = await server.inject().post("/users").body(createObj);
  ...
  it("get users GET /users", async () => {
    ...
    const res = await server.inject().headers(headers).get("/users");
